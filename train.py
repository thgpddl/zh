...
def main():
    ...
    if cm:
        task = Task.init(project_name="HP-Fer2013-zh", task_name=config["name"])  # 负责记录输入输出
        task.connect(config)
    ...
    train_loader, val_loader, test_loader = get_dataloaders(path=config['data_path'],
    ...
    test(model, test_loader, config['Ncrop'], device)
